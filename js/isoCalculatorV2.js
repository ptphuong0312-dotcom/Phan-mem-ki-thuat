...
const IsoCalcV2 = (function() {
    ...
    return {
        ...
        calculateFit: function(size, holeLetter, holeIT, shaftLetter, shaftIT) {
            ...
            let maxInterference = -minClearance;
            let minInterference = -maxClearance;
            ...
            return { hole, shaft, maxClearance, minClearance, maxInterference, minInterference, type };
        },
...
window.runIsoCalcV2 = function() {
    ...
    // Fractional elements
    const fvcContainer = document.getElementById('calcFvcContainer');
    const fvcNom = document.getElementById('calcFvcNominal');
    const fvcHoleCls = document.getElementById('calcFvcHoleClass');
    const fvcHoleES = document.getElementById('calcFvcHoleES');
    const fvcHoleEI = document.getElementById('calcFvcHoleEI');
    const fvcShaftCls = document.getElementById('calcFvcShaftClass');
    const fvcShaftEs = document.getElementById('calcFvcShaftEs');
    const fvcShaftEi = document.getElementById('calcFvcShaftEi');
    
    // Fit App info
    const calcFitApp = document.getElementById('calcFitApp');
    const getFitAppStr = typeof getFitApplicationHtml === 'function' ? getFitApplicationHtml : () => '';
    ...
    if (parsed.mode === 'single') {
        ...
        if (!res) {
            ...
            fvcContainer.style.display = 'none';
            calcFitApp.style.display = 'none';
        ...
        fvcContainer.style.display = 'flex';
        fvcNom.innerText = parsed.size;
        
        if (parsed.isHole) {
            fvcHoleCls.innerText = parsed.letter + parsed.it;
            fvcHoleES.innerText = fmt(res.ES, 3, true);
            fvcHoleEI.innerText = fmt(res.EI, 3, true);
            fvcHoleES.style.visibility = 'visible';
            fvcHoleEI.style.visibility = 'visible';
            
            fvcShaftCls.innerText = '-';
            fvcShaftEs.style.visibility = 'hidden';
            fvcShaftEi.style.visibility = 'hidden';
        } else {
            fvcShaftCls.innerText = parsed.letter + parsed.it;
            fvcShaftEs.innerText = fmt(res.es, 3, true);
            fvcShaftEi.innerText = fmt(res.ei, 3, true);
            fvcShaftEs.style.visibility = 'visible';
            fvcShaftEi.style.visibility = 'visible';
            
            fvcHoleCls.innerText = '-';
            fvcHoleES.style.visibility = 'hidden';
            fvcHoleEI.style.visibility = 'hidden';
        }
        const typeObj = document.getElementById('calcFitType');
        if(typeObj) typeObj.style.display = 'none';
        const clearanceInfo = document.getElementById('calcClearanceInfo');
        if(clearanceInfo) clearanceInfo.style.display = 'none';
        
        calcFitApp.style.display = 'none';
        const svgW = document.getElementById('calcSvgWrapper');
        ...
        if(typeObj) {
            ...
            if (fit.type.includes('DÔI')) badgeClass = 'red';
            else if (fit.type.includes('TRUNG GIAN')) badgeClass = 'orange';
            typeObj.className = `fit-type-badge ${badgeClass}`;
            ...
            if (clearanceInfo) {
                let infoHtml = '';
                if (badgeClass === 'green') {
                    infoHtml = `Độ hở lớn nhất = ${fmt(fit.maxClearance)}<br>Độ hở nhỏ nhất = ${fmt(fit.minClearance)}`;
                } else if (badgeClass === 'red') {
                    infoHtml = `Độ dôi lớn nhất = ${fmt(fit.maxInterference)}<br>Độ dôi nhỏ nhất = ${fmt(fit.minInterference)}`;
                } else {
                    infoHtml = `Độ hở lớn nhất = ${fmt(fit.maxClearance)}<br>Độ dôi lớn nhất = ${fmt(fit.maxInterference)}`;
                }
                clearanceInfo.innerHTML = infoHtml;
                clearanceInfo.style.display = 'block';
            }
        }
        ...
        fvcHoleCls.innerText = parsed.hole.letter + parsed.hole.it;
        fvcHoleES.innerText = fmt(fit.hole.ES, 3, true);
        fvcHoleEI.innerText = fmt(fit.hole.EI, 3, true);
        fvcHoleES.style.visibility = 'visible';
        fvcHoleEI.style.visibility = 'visible';
        
        fvcShaftCls.innerText = parsed.shaft.letter + parsed.shaft.it;
        fvcShaftEs.innerText = fmt(fit.shaft.es, 3, true);
        fvcShaftEi.innerText = fmt(fit.shaft.ei, 3, true);
        fvcShaftEs.style.visibility = 'visible';
        fvcShaftEi.style.visibility = 'visible';
        
        const fitAppStr = getFitAppStr(parsed.hole.letter + parsed.hole.it, parsed.shaft.letter + parsed.shaft.it);
        if (fitAppStr) {
            calcFitApp.innerHTML = fitAppStr;
            calcFitApp.style.display = 'block';
        } else {
            calcFitApp.style.display = 'none';
        }
        
        if (typeof renderFitChart === 'function') {
            const svgW = document.getElementById('calcSvgWrapper');
            if(svgW) svgW.style.display = 'block';
            renderFitChart(fit, 'calcSvgHoleRect', 'calcSvgHoleLabel', 'calcSvgShaftRect', 'calcSvgShaftLabel');
        }
    }
};
