...
## Hệ Thống Ma Trận Lỗ & Trục Cơ Bản (Basic Fit Systems)
- **js/data.js**: Chứa `HOLE_SYS_MATRIX` và `SHAFT_SYS_MATRIX`. Đây là ma trận lắp ghép gồm các kiểu lắp ưu tiên (preferred), phổ biến (popular), tiêu chuẩn (standard). Dữ liệu này cung cấp sẵn các cặp lỗ/trục ghép với nhau.
- **js/isoUI.js**: Hàm `initHoleSysPanel()` và `initShaftSysPanel()` phụ trách tạo HTML (thẻ bảng `<table>` và các tag kiểu lắp ghép). Đồng thời quản lý sự kiện chọn kiểu ghép thông qua `selectHoleSysPair()` và `selectShaftSysPair()`. 
- Khi người dùng click, bảng giải thích (Fit Application) từ `fitApplications.js` sẽ được gọi ra thông qua `getFitApplicationHtml()`.
