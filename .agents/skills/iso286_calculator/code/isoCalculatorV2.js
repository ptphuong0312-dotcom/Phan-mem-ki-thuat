...
window.runIsoCalcV2 = function() {
    ...
    document.getElementById('drawNominal').innerText = size;

    const holeWrap = document.getElementById('drawHoleWrapper');
    const shaftWrap = document.getElementById('drawShaftWrapper');
    const divWrap = document.getElementById('drawDivider');
    const badgeWrap = document.getElementById('drawFitBadge');
    const detWrap = document.getElementById('drawFitDetailsWrapper');

    holeWrap.style.display = validHole ? 'flex' : 'none';
    shaftWrap.style.display = validShaft ? 'flex' : 'none';
    divWrap.style.display = (validHole && validShaft) ? 'block' : 'none';
    badgeWrap.style.display = (validHole && validShaft) ? 'inline-block' : 'none';
    detWrap.style.display = (validHole && validShaft) ? 'inline-block' : 'none';
    ...
    if (parsed.mode === 'single') {
        ...
        if (parsed.isHole) {
            document.getElementById('drawHoleClass').innerText = parsed.letter + parsed.it;
            document.getElementById('drawHoleES').innerText = fmt(res.ES, 3, true);
            document.getElementById('drawHoleEI').innerText = fmt(res.EI, 3, true);
        } else {
            document.getElementById('drawShaftClass').innerText = parsed.letter + parsed.it;
            document.getElementById('drawShaftEs').innerText = fmt(res.es, 3, true);
            document.getElementById('drawShaftEi').innerText = fmt(res.ei, 3, true);
        }
        ...
        document.getElementById('drawHoleClass').innerText = parsed.hole.letter + parsed.hole.it;
        document.getElementById('drawHoleES').innerText = fmt(fit.hole.ES, 3, true);
        document.getElementById('drawHoleEI').innerText = fmt(fit.hole.EI, 3, true);
        
        document.getElementById('drawShaftClass').innerText = parsed.shaft.letter + parsed.shaft.it;
        document.getElementById('drawShaftEs').innerText = fmt(fit.shaft.es, 3, true);
        document.getElementById('drawShaftEi').innerText = fmt(fit.shaft.ei, 3, true);

        let cls = 'green';
        if (fit.type === 'LẮP CHẶT') cls = 'red';
        else if (fit.type === 'LẮP TRUNG GIAN') cls = 'yellow';

        badgeWrap.innerText = fit.type.toUpperCase();
        badgeWrap.className = `fit-type-badge ${cls}`;

        let detailsHtml = '';
        if (cls === 'green') {
            detailsHtml = `
                <div style="color: #fff;"><span style="color: #e2e8f0; font-weight: normal;">Độ hở lớn nhất (S_max):</span> ${fmt(fit.maxClearance)} <span style="color: #94a3b8; font-weight: normal;">mm</span></div>
                <div style="color: #fff;"><span style="color: #e2e8f0; font-weight: normal;">Độ hở nhỏ nhất (S_min):</span> ${fmt(fit.minClearance)} <span style="color: #94a3b8; font-weight: normal;">mm</span></div>
            `;
        } else if (cls === 'red') {
            detailsHtml = `
                <div style="color: #fff;"><span style="color: #e2e8f0; font-weight: normal;">Độ dôi lớn nhất (N_max):</span> ${fmt(fit.maxInterference)} <span style="color: #94a3b8; font-weight: normal;">mm</span></div>
                <div style="color: #fff;"><span style="color: #e2e8f0; font-weight: normal;">Độ dôi nhỏ nhất (N_min):</span> ${fmt(fit.minInterference)} <span style="color: #94a3b8; font-weight: normal;">mm</span></div>
            `;
        } else {
            detailsHtml = `
                <div style="color: #fff;"><span style="color: #e2e8f0; font-weight: normal;">Độ hở lớn nhất (S_max):</span> ${fmt(fit.maxClearance)} <span style="color: #94a3b8; font-weight: normal;">mm</span></div>
        ...
        document.getElementById('drawFitDetails').innerHTML = detailsHtml;
